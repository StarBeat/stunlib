...
#define __CTEST2_INTERNAL(sname, tname, _skip) \
  ...
  __CTEST_STRUCT( sname, tname, _skip, &__ctest_ ## sname ## _data, \
                  SETUP_FNAME(sname), TEARDOWN_FNAME(sname) ) \
  void __FNAME(sname, tname)(struct sname ## _data* data)
            ...
            int                  line)
{
  ...
  if (expsize != realsize)
  {
    CTEST_ERR("%s:%d  expected %d bytes, got %d",
              caller,
              line,
              expsize,
              realsize);
           ...
           const char* argv[])
{
  ...
  char        results[80];
  sprintf(results,
          "RESULTS: %d tests (%d ok, %d failed, %d skipped) ran in %" PRIu64 " ms",
          total,
          num_ok,
          num_fail,
          num_skip,
